...
hidden_dim = 384
capacity = 1000000
fade_factor = 20 # fading memory factor, 7 -remembers ~30% of the last transtions before gradual forgetting, 1 - linear forgetting, 10 - ~50% of transitions, 100 - ~70% of transitions.
tau = 0.005
...
algo = Symphony(state_dim, action_dim, hidden_dim, device, max_action, tau, capacity, fade_factor)
...
for i in range(start_episode, num_episodes):
    ...
    for steps in range(1, limit_step+1):
        ...
        if (total_steps>=2500 and total_steps%2500==0): testing(env_test, limit_step=limit_eval, test_episodes=100, current_step=total_steps, save_log=True)
