...
class Swaddling(jit.ScriptModule):
    ...
    def forward(self, x, k):
        ...
        sw = (self.Omega(x**(1/k.detach())) + k * self.omega(x/math.e)).sum(dim=-1, keepdim=True)
...
class FeatureExtractor(jit.ScriptModule):
    ...
    def za(self, s, a):
        ...
        return self.norm2(torch.cat([za, self.rew.latent(za)], dim=-1))
...
class Critic(jit.ScriptModule):
    def __init__(self, state_action_dim, z_dim, q_dist, drop=True):
        ...
        self.God = FeedForward(state_action_dim, z_dim, q_dist, drop)
        self.h1 = nn.Linear(z_dim, q_nodes)
        self.h2 = nn.Linear(z_dim, q_nodes)
        self.h3 = nn.Linear(z_dim, q_nodes)
        self.h4 = nn.Linear(z_dim, q_nodes)

        self.heads = nn.ModuleList([self.h1, self.h2, self.h3, self.h4])

 

    @jit.script_method
    def forward(self, state_action):
        x = self.ffw.latent(state_action)
...
class ActorCritic(jit.ScriptModule):
    ...
    def critic_direct(self, state, action):
        return self.critic(self.fe.za(state, action))
...
class Nets(jit.ScriptModule):
    ...
    def update(self):
        ...
        q_pred = self.online.critic_direct(state, action)


        net_loss = self.rehse(q_pred-q_target) - self.rehae((q_next_target - q_next_ema)/q_next_ema.abs()) + sw_and_beta_loss
        net_loss.backward()
