...
class ReHSE(jit.ScriptModule):
    ...
    def forward(self, e):
        ae = torch.abs(e) + 1e-6
        ae = torch.sqrt(ae)*torch.tanh(ae/4)
        return ae.mean()
...
class ReHAE(jit.ScriptModule):
    ...
    def forward(self, e):
        e = e + 1e-6
        e = torch.sqrt(torch.abs(e))*torch.tanh(e/4)
        return e.mean()
...
class ActorCritic(jit.ScriptModule):
    def __init__(self, state_dim, action_dim, max_action=1.0):
        ...
        self.q = FeedForward(state_dim+action_dim, 128)
    ...
    def actor(self, state, noise:bool=True):
        ...
        if noise: x[:,1] += 0.1 * self.a_max * torch.randn_like(x[:,1]).clamp(-3.0, 3.0)
...
class Symphony(object):
    ...
    def update(self):
        ...
        nets_loss = -self.rehae(q_next_target-q_next_ema) + self.rehse(q-qs)
